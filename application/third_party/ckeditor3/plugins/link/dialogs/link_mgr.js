...
CKEDITOR.dialog.add( 'link', function( editor )
{
	...
	/*PHP'S function strtr.. to clean internal link*/
	...
	var javascriptProtocolRegex = /^javascript:/,
		...
		emailBodyRegex = /body=([^;?:@&=$,\/]*)/,
		anchorRegex = /^#(.*)$/,
		internalRegex = /wakka=([A-Za-z0-9_]+)/,
	...
	var parseLink = function( editor, element )
	{
		...
		if ( !retval.type )
		{
			...
			else if ( href && ( internalMatch = href.match( internalRegex ) ) )
			{
				retval.type = 'internal';
				retval.internal = {};
				retval.internal.pageName = internalMatch[1];
			}
				...
				[
					{
						...
						items :
						[
							[ linkLang.internal, 'internal' ],
							[ linkLang.external, 'url' ],
						...
						[
							{
								type : 'text',
								id : 'pageName',
								label : editor.lang.link.pageName,
								validate : function()
								{
									//todo: nospecial char 									
									var dialog = this.getDialog();
									...
									if ( !dialog.getContentElement( 'info', 'linkType' ) ||
											dialog.getValueOf( 'info', 'linkType' ) != 'internal' )
										return true;

									var func = CKEDITOR.dialog.validate.notEmpty( editor.lang.link.noPage );
									return func.apply( this );
								},
								setup : function( data )
								{
									if ( data.internal )
										this.setValue( data.internal.pageName );
								...
								commit : function( data )
								{
									...
									data.internal.pageName = this.getValue();
								}
							}
						],
						setup : function( data )
		...
		onOk : function()
		{
			...
			switch ( data.type || 'url' )
			{
				case 'url':
					...
					break;
				case 'internal':
					var name = ( data.internal && data.internal.pageName ),
						id = ( data.internal && data.internal.pageName ),
						out = strtr(id,'ÉÈÊÇÀÂÎÏÙÔéèêçàâîïùô ','EEECAAIIUOeeecaaiiuo_');
						out = out.replace(/\W/g,"_");
						
						name = out;
						id = out;
						
					attributes.href = 'wikka.php?wakka=' + (out || '' );
					break;
				case 'anchor':
					var name = ( data.anchor && data.anchor.name ),
						id = ( data.anchor && data.anchor.id );
					attributes.href = '#' + ( name || id || '' );
					break;
				case 'email':
