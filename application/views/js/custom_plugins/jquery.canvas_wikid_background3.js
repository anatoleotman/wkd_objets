...
;			this.$elem_cadre_back = $('#cadre_back'); // pour changer la couleur du cadre lors de l'affichage du canvas
		...
		eventify: function () {
			this.$elem.on('mouseenter', 'big', $.proxy(this.handler_on_mouseenter, this));
			this.$elem.on('mouseleave', 'big', $.proxy(this.handler_on_mouseleave, this));
		},
		
		handler_on_mouseenter: function (event) {
			if (this.time_out_pedalo_ivre_background) {
				clearTimeout(this.time_out_pedalo_ivre_background);
			}
			//console.info(event.currentTarget);
			var $image = $('img', event.currentTarget);
			if ($image.length) {
				...
				var ctx = canvas[0].getContext("2d");
    				// create pattern
       				...
       				var $that_elem_cadre_back = this.$elem_cadre_back;
       				canvas.show('slide', {direction: case_direction}, 'easeIn', function () {
       					$that_elem_cadre_back.css('background-color', 'rgba(255, 255, 255, 0.8)');
       				});
		...
		handler_on_mouseleave: function (event) {
			...
			if ($image.length) {
				...
				if ( (y - h*x/w) < 0 && (y + h*x/w - h) < 0) {case_direction = 'up';}
				var $that_elem_cadre_back = this.$elem_cadre_back;
				$('#canvas_background').hide('slide', {direction: case_direction}, 'easeOut', function () {
       					$that_elem_cadre_back.css('background-color', 'rgba(0, 0, 0, 0.05)');
       				});
				this.time_out_pedalo_ivre_background = setTimeout(
					$.proxy(this.affiche_pedalo_ivre_canvas_background, this), 
					900
				);
				console.info(case_direction);
			}
		...
		affiche_pedalo_ivre_canvas_background: function () {
			this.$elem_cadre_back.css('background-color', 'rgba(255, 255, 255, 0.8)');
			var image_url = WIKIDGLOBALS.BASE_URL + 'application/images/PedaloIvre_alpha50.png';	
			$('#body_principal').css("background-image", "url(" + image_url + ")");
		},
		...
		images_build_canvas_on_mouse_over: function (elem_ref) {
			...
			var affiche_pedalo_ivre_canvas_background = function () {
				...
				that.$elem_cadre_back.animate({ backgroundColor: 'rgba(255, 255, 255, 0.8)'}, 'slow');
			};
			...
			if ($big_ref.length) {
				$('img', $big_ref)
					...
					.mouseleave(function () {
						...
						that.$elem_cadre_back.animate({ backgroundColor: 'rgba(0, 0, 0, 0.05)'}, 'slow');
