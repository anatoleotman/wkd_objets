...
/*destroy_plugin_instance: function () {
		...
		bouton_edit_transfer_effect_init: function () {
			var that = this;
			this.$bouton_edit_mode
				.button('widget')
				.one('mouseenter.transfer_effect', function () {
	//				$('.ui-effects-transfer').css('background', that.string_couleur_target_rgba);
					...
					$(this).effect('transfer', {to: that._get_elem_contenu_ckeditable(), className: 'transfer_effect_custom' }, 800, function () {
						$(this).stop('true');
...
//				}).appendTo(this.$elem);
//				
			this.$elem.on('mousemove.bouton_position_effect', this._get_elem_contenu_ckeditable(), function (mouseOver_event) {
...
//				that.$elem.find('.pageCoords').text("( e.pageX, e.pageY ) : " + pageCoords);
//				that.$elem.find('.clientCoords').text("( e.clientX, e.clientY ) : " + clientCoords);
//				that.$bouton_edit_mode
//					.button('widget')
//					.stop('true')
//					.position({
//						my: "top center",
//						at: "bottom left+"+relX.toString(),
//						of: $target_elem,
//						using: function (css, calc) {
//							that.$bouton_edit_mode
//								.button('widget')
//								.animate(css, "400", "easeOutQuint");
//					}
//				});
				that.$bouton_edit_mode
					.button('widget')
...
//			this.$elem.on('mouseout', this._get_elem_contenu_ckeditable(), function (mouseleave_event) {
//				that.$bouton_edit_mode
//					//.button('widget')
//					.stop()
//					.position({
...
/*this.$wikid_form.load(	WIKIDGLOBALS.BASE_DIRECTORY + 
			...
			$.ajax({
				...
				complete: function () {
					...
					this.$bouton_edit_mode.stop('true');
					this.$bouton_edit_mode.off('.transfer_effect');
					this.$elem.off('mousemove.bouton_position_effect');
						// cacher le bouton edit_mode
				//this.$bouton_edit_mode.stop('true');
				//this.$bouton_edit_mode.hide('scale');
					this._init_ckeditor();
				}
			});
		},
		...
		_init_ckeditor: function () {
			...
			this.$elem.find('a').on('click.div_wikidable', function () {
				event.preventDefault();
				event.stopImmediatePropagation();
			});
			...
			this.$bouton_edit_mode.effect("transfer", {
				to: this._get_elem_contenu_ckeditable(), className:'transfer_effect_custom'
			}, 800, function () {
...
//			$('.ui-effects-transfer').css('background', this.string_couleur_target_rgba);
		...
		_close_ckeditor: function () {
			var that = this;
			this.$elem.find('a').off('click.div_wikidable');
