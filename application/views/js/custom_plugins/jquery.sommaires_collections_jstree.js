...
//				});
			var random_color1 = Math.floor(Math.random()*256);
   			...
   			var string_couleur_target_rgba = 'rgba(' 
   					...
   					+ random_color3 + ',' 
   					+ random_color4 + ')';
   			var string_couleur_target_rgba2 = 'rgba(' 
   					+ random_color1 + ',' 
   					+ random_color2 + ',' 
   					+ random_color3 + ',' 
   					+ random_color5 + ')';	
   			var string_couleur_target_rgba3 = 'rgba(' 
   					+ random_color1 + ',' 
   					+ random_color2 + ',' 
   					+ random_color3 + ',' 
   					+ random_color6 + ')';			
		...
		// construire les boutons 	
			$('#buttonset_sommaire_collection')
				.append(this.$add_folder_bouton)
				.append(this.$add_object_bouton)
				.append(this.$view_collection_bouton)
				.buttonset();
...
//					}
		...
		handler_new_object: function () {
			...
			var $new_obj_elem = $('<div>', {
				...
				.load(WIKIDGLOBALS.BASE_DIRECTORY + "index.php/collection_objets/display_new_object_template/" + $current_page_name)
				.appendTo(this.$elem);
		...
		ajax_form_new_object: function (elem) {
			$(elem).find('#new_object_form').validate({
				submitHandler: function (form) {
					...
					function () { alert('une entrée a été ajoutée au sommaire')}, 
					true			);
								$(this).parent().remove();
...
//			this.$elem.find('[contenteditable="true"]').ckeditor('inline');
		...
		handler_view_collection: function () {
				...
				.load(WIKIDGLOBALS.BASE_DIRECTORY + "index.php/collection_objets/display_collection/" + $current_page_name)
				.appendTo(this.$elem)
				.show('slide', function () {
					$(this).jstree({
				"ui": {
