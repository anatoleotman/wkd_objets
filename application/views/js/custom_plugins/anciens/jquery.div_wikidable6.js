...
(function ($) {
	var Wrapper_div_Wikidable = {
		init: function (options, elem) {
			...
			this.elem = elem;
			this.$elem = $(elem);// cest le wrapper wikid
			
			this.$elem_contenu_wikid = $('.contenu_wikid', $(elem));//une reference au div contenu wikid
		...
		handler_reset_button: function (event) {
			CKEDITOR.instances[this.$wikid_form_textarea.attr('name')].updateElement();
			console.info('reset bouton clicked ck editor update ??');
		},
			...
			$.ajax({
				...
				beforeSend: function () {
			  	...
			  	success: function (ans) {			  	
					...
					this._init_wikid_form_ajax(); // ajaxform plugin init
			 	},
			 	complete: function () {
		...
		_init_wikid_form_ajax: function () {
			...
			$(':reset', this.$wikid_form).on('click', $.proxy(this.handler_reset_button, this));
			...
			this.$wikid_form.ajaxForm({
				...
				success: function (ans) {
					this.$elem_contenu_wikid.html(ans);
					this._close_ckeditor();
				},
				complete: function () {
					this.$elem.spin(false);
