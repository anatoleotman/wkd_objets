...
//il faut charger jstree.js
//le plugin est écrit sous forme de object literal'




//closure et fonction anonyme: c'est la définiton du plugin jquery'
(function ($) {
	'use strict';
	...
	var Jstree_menu_wikid0 = {
		init: function (options, elem) {
			...
			this.options = $.extend({}, this.options, options); //this renvoie à l'objet literal ici c'est Jstree_menu_wikid0'
		...
		_build_element: function () {
			this.$elem.append('<span id="menu_tree" style="display:none;"></span>'); //menu_tree un id à passer en options par exemple !?'
			this.$menu_tree = this.$elem.find('#menu_tree'); //on garde la référence jquery du conteneur créé pour le jstree
			//return this; pas la peine de chainer c'est une fonction privée/interne: on l'appelle de l'intérieur de l'objet '
		},

		_init_jstree: function () { //on pourrait l'appeler dans init mais pourquoi pas l'appeler au moment du dblclick'
			var that = this;
		...
		refresh_menu_ajax: function () {
			...
			$.ajax({
				...
				success : function (ans) {
					if (!ans.success) {alert('Erreur durant le chargement !'); } else {
						var dumm_menu = document.getElementById('menu');
						purge(dumm_menu);
						dumm_menu.innerHTML = '';// parce que empty ou remove ça déconnait
						console.info(dumm_menu);
						that.$elem_menu.html(ans.menu); //html ou empty.append les deux semblent fonctionner
						that.options.on_refresh_callback();
					}
				}
			});
		}
