(function ($) {
	var Canvas_wikid_background_Prototype = {
		...
		StackBlurForCanvas: {
			...
			stackBlurImage: function( image_elem_ref, canvas_elem_ref, radius, blurAlphaChannel, globalAlpha )	{
			
			 	var img = image_elem_ref;//document.getElementById( imageID );
				var w = img.naturalWidth;
			    	var h = img.naturalHeight;
			       
				var canvas = canvas_elem_ref;//document.getElementById( canvasID );
				...
				if ( blurAlphaChannel )
					this.stackBlurCanvasRGBA( canvas_elem_ref, 0, 0, w, h, radius );
				else 
					this.stackBlurCanvasRGB( canvas_elem_ref, 0, 0, w, h, radius );
			},


			stackBlurCanvasRGBA: function ( elem_ref, top_x, top_y, width, height, radius ) {
				if ( isNaN(radius) || radius < 1 ) return;
				radius |= 0;
	
				var canvas  = elem_ref;//document.getElementById( id );
				var context = canvas.getContext("2d");
				var imageData;
			...
			stackBlurCanvasRGB: function ( elem_ref, top_x, top_y, width, height, radius ) {
				if ( isNaN(radius) || radius < 1 ) return;
				radius |= 0;
	
				var canvas  = elem_ref; //ocument.getElementById( id );
				var context = canvas.getContext("2d");
				var imageData;
